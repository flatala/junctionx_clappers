...
def process_remove(file_path: Path, spans: List[dict]) -> Path:
    ...
    if _is_video(file_path):
        ...
        if not kept_clips:
            black = ColorClip(size=(clip.w, clip.h), color=(0, 0, 0), duration=0.5)
            silent_audio = AudioSegment.silent(duration=500)
            tmp_audio = tempfile.NamedTemporaryFile(delete=False, suffix=".mp3")
            silent_audio.export(tmp_audio.name, format="mp3")
            tmp_audio.close()
            black = black.set_audio(AudioFileClip(tmp_audio.name))
...
def clean(
    ...
    for job in jobs:
        ...
        #     print(f"[WARNING] Skipping job {job.id} with status {job.status}")
        #     continue
        ...
        spans_path = processed_dir / (file_path.stem + "_processed_spans.json")
